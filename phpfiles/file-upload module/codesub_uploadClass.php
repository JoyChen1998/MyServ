...
class FileUpload {
    ...
    private $allowtype = array('jpg','gif','png','zip','doc','docx','txt'); //设置限制上传文件的类型
    private $maxsize = 1000000;           //限制文件上传大小（字节）
    private $setname = true;           //设置是否随机重命名文件， false不随机
    // $sessionID = isset($_SESSION['id'])?$_SESSION['id']:0;      //获取session中的ID,default_id = 0
    ...
    /**
     * 用于设置成员属性（$path, $allowtype,$maxsize, $setname）
     * 可以通过连贯操作一次设置多个属性值
     ...
     * 获取上传后的文件名称
     * @param  void   没有参数
     * @return string 上传后的文件的名称， 如果是多文件上传返回数组
     */
...
//        $str = "<font color='red'>{$this->originName}</font>";
    ...
    private function setNewFileName() {
        if ($this->setname) {
            $this->setOption('newFileName', $this->proStudentName());
        } else{
    ...
    /* 设置文件名 */
    private function proStudentName() {
